...
export async function generateEventsExcelReport(events: any[]) {
  ...
  const activeEvents = events.filter(e => {
    const status = getEventStatusText(e).toLowerCase();
    return !status.includes('cancelado') && !status.includes('anulado');
  });
  ...
  for (let i = 0; i < activeEvents.length; i += BATCH_SIZE) {
    ...
    await Promise.all(batch.map(async (event) => {
      ...
      try {
        ...
        console.warn(`No se pudo obtener la cotización para el evento ${event.idEvent}:`, error);
      }
        ...
        activities.forEach((activity: any) => {
          ...
          if (activity.services && Array.isArray(activity.services)) {
            activity.services.forEach((service: any) => {
              ...
              const { net, discount, price } = calculateItemAmounts(service, quantity);
              ...
              if (net > 0 || price > 0) {
                ...
                const revenueGroupName = catalogService?.revenueGroup?.revenueGroupName?.toLowerCase() || '';
                const isGastronomia = 
      ...
      reportData.push({
        ...
        'Fecha Inicial': event.startDate ? new Date(event.startDate).toLocaleDateString('es-ES') : '',
        'Fecha Final': event.endDate ? new Date(event.endDate).toLocaleDateString('es-ES') : '',
  ...
  XLSX.writeFile(workbook, `Reporte_Eventos_${new Date().toISOString().split('T')[0]}.xlsx`);
}
...
export async function generatePersonalExcelReport(events: any[]) {
        ...
        activities.forEach((activity: any) => {
          ...
          if (activity.services && Array.isArray(activity.services)) {
            activity.services.forEach((service: any) => {
              ...
              const isPersonal = personalKeywords.some(keyword => serviceName.includes(keyword));
              ...
              if (isPersonal) {
                ...
                if (serviceName.includes("auxiliar de limpieza") || serviceName.includes("auxiliar de aseo")) {
                  normalizedName = "Auxiliar de Limpieza";
                } else if (serviceName.includes("auxiliar de montajes")) {
                  normalizedName = "Auxiliar de Montajes";
                } else if (serviceName.includes("oficial") && serviceName.includes("proteccion")) {
                  normalizedName = "Oficial Gestión de la Protección";
                }

                const quantity = service.quantity || service.serviceQuantity || 0;
  ...
  if (reportData.length === 0) {
    throw new Error('No se encontró personal asignado en los eventos seleccionados.');
  }
...
export async function generateParqueosExcelReport(events: any[]) {
  ...
  if (reportData.length === 0) {
    throw new Error('No se encontraron servicios de parqueo en los eventos seleccionados.');
  }
  ...
  // Generar el archivo
  XLSX.writeFile(workbook, `Reporte_Parqueos_${new Date().toISOString().split('T')[0]}.xlsx`);
}
